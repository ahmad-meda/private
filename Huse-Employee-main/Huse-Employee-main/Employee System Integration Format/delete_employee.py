...
def soft_delete_employee(contact_number, user_message:str):
    ...
    extracted_data = soft_delete_employee_extraction(messages=extraction_messages[-2:])
    ...
    if extracted_data.employee_name:
        ...
        employee_name = EmployeeProxy.get_employee_id_by_name(name=extracted_data.employee_name)
    ...
    if extracted_data.contact_number:
        employee_id = EmployeeProxy.get_employee_id_by_contact(contact_number=extracted_data.contact_number)
    ...
    response = soft_delete_employee_response(messages=extraction_messages[-1:], fields=agent_states.mandatory_fields, error_dict=error_dict)
