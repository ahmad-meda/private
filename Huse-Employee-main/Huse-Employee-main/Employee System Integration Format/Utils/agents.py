    ...
    system_message = [
        {
            "role": "system",
            "content": (
                    """Extract employee identification details from the ENTIRE conversation history for deletion purposes.
                    
                    IMPORTANT RULES:
                    - Look through ALL messages for employee identification information
                    - Extract employee names from various formats like "name is John", "employee name: John", "John Smith", "delete John", etc.
                    - Extract contact numbers with or without country codes
                    - Use exact values as provided by the user
                    - For missing information, use None
                    - Be thorough - check every message for identification details
                    
                    FIELDS TO EXTRACT:
                    - employee_name: Complete name of the employee to be deleted (extract from phrases like "name is...", "employee name:", direct name mentions, etc.)
                    - contact_number: Phone/contact number of the employee to be deleted (with country code if provided)
                    
                    EXAMPLES:
                    - "the name is Ali Mohammed" → employee_name: "Ali Mohammed"
                    - "delete John Smith" → employee_name: "John Smith"  
                    - "employee name: Sarah" → employee_name: "Sarah"
                    - "contact is +971501234567" → contact_number: "+971501234567"
                    """
            )
        }
    ] + messages
...
def locate_update_employee(messages:list, employee_record: Employee):

    system_message = [
        {
            "role": "system",
            "content": (
                                # these are the details of the employee chatting with you: {employee_record}. Only use these details when the employee chatting with you says he wants to update his own details.
    ...
    system_message = [
        {
            ...
            "content": (
                f"""
                You are a data extraction assistant used to identify which field the user wants to update. Change the fields that the user want to update to true.
                """
            )
        }
    ] + messages
