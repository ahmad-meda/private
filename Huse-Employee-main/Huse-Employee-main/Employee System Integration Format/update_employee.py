...
def update_employee_fields(contact_number:str, user_message:str):
    ...
    extracted_current_data = locate_update_employee(messages=sanitized_messages, employee_record=employee_record)
    ...
    if employee_identified == True:
        ...
        response = update_employee_response(messages=extraction_messages[-4:], employee_identified=employee_identified, error_dict=error_dict, fields=agent_states.mandatory_fields)
        LeadMessageHistoryProxy.save_message(contact_number, "assistant", response.message_to_user)
        EmployeeSessionProxy.add_message(contact_number, sanitize_messages(session_messages))
        # send_whatsapp_message(contact_number=contact_number, response=response.message_to_user)
        print(f"chat_assistant: {response}")
...
#     update_employee_fields(contact_number="+971501234567", user_message=user_input)