...
class EmployeeService:
    ...
    @staticmethod
    def get_company_choices(db: Session, group_id: int):
    ...
    @staticmethod
    def get_employee_id_by_contact(contactno: str, db_session: Session):
        response = {
        "success": False,
        "database_id": None,
        "error": None
        ...
        try:
            ...
            employees = db_session.query(Employee).filter(Employee.is_deleted == False).all()
    ...
    @staticmethod
    def get_employee_id_by_name(name: str, email: Optional[str], contact_number: Optional[str], db_session: Session):
        ...
        try:
            # Get all non-deleted employees with non-null names
            employees = db_session.query(Employee).filter(Employee.is_deleted == False).all()
            employee_data = [
                {
                    "name": emp.name,
...
#--------------------------------------------GET OPERATIONS--------------------------------------------
    ...
    def get_employee_by_company(db: Session, company_name: str, hr_company_id: int = None, group_id: int = None):
        try:
            company_name = find_best_match(company_name, EmployeeService.get_company_choices(db, group_id), threshold=85)
        ...
        try:
            ...
            for field, value in criteria_list:
                if field == "name":
                    ...
                    matched_company = find_best_match(value, EmployeeService.get_company_choices(db, group_id), threshold=85)
