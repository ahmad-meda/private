...
class EmployeeProxy:
    ...
    @staticmethod
    def get_company_choices(group_id: int):
        from app import app, db
        with app.app_context():
    ...
    @staticmethod
    def get_employee_id_by_contact(contact_number: str):
        from app import app, db
        with app.app_context():
            return EmployeeService.get_employee_id_by_contact(contact_number, db.session)
    
    @staticmethod
    def get_employee_id_by_name(name: str, email: str = None, contact_number: str = None):
        from app import app, db
        with app.app_context():
            return EmployeeService.get_employee_id_by_name(name, email, contact_number, db.session)
